...
try:
    if __name__ == '__main__':
        ...
        parser.add_argument('--file', type=str, default='./acc_result/acc_cifar')
        ...
        if (args.noise > 0):
            ...
            prune.random_unstructured(G.emb, name="weight", amount=0.5)
            prune.random_unstructured(G.linear_1, name="weight", amount=0.5)
            prune.random_unstructured(G.linear_2, name="weight", amount=0.5)
            prune.random_unstructured(G.conv_tran_1, name="weight", amount=0.5)
            prune.random_unstructured(G.conv_tran_2, name="weight", amount=0.5)
            prune.random_unstructured(G.conv, name="weight", amount=0.5)
            prune.random_unstructured(D.emb, name="weight", amount=0.5)
            prune.random_unstructured(D.linear_1, name="weight", amount=0.5)
            prune.random_unstructured(D.linear_2, name="weight", amount=0.5)
            prune.random_unstructured(D.conv_1, name="weight", amount=0.5)
            prune.random_unstructured(D.conv_2, name="weight", amount=0.5)
            ...
            print(dire)
            fl1 = open("./checkpoint_cifar/"+dire+"/diff_iter/acc_different_iter.txt","w")
            fl2 = open("./checkpoint_cifar/"+dire+"/diff_acc/iter_acc.txt","w")
            fl3 = open("./checkpoint_cifar/"+dire+"/record.txt","w")            
            ...
            for iteration, (x_mini, y_mini) in bar:
                ...
                if((iteration+1) %100 ==0):
                    ...
                    ##remove pruning before save model
                    prune.remove(G.emb, name="weight")
                    prune.remove(G.linear_1, name="weight")
                    prune.remove(G.linear_2, name="weight")
                    prune.remove(G.conv_tran_1, name="weight")
                    prune.remove(G.conv_tran_2, name="weight")
                    prune.remove(G.conv, name="weight")
                    prune.remove(D.emb, name="weight")
                    prune.remove(D.linear_1, name="weight")
                    prune.remove(D.linear_2, name="weight")
                    prune.remove(D.conv_1, name="weight")
                    prune.remove(D.conv_2, name="weight")
                    ...
                    if((iteration+1) in log2):
                        ...
                        torch.save(G.state_dict(), f"./checkpoint_cifar/"+dire+f"/diff_iter/iteration{(iteration+1)}.ckpt")
                    ...
                    for i in range(len(log1)):
                        if(test_acc/test_set.__len__() >log1[i] and log1_[i] ==False):
                            ...
                            torch.save(G.state_dict(), f"./checkpoint_cifar/"+dire+f"/diff_acc/acc{aaa:.1f}.ckpt")
                    del C
            ...
            prune.remove(D.conv_1, name="weight")
            prune.remove(D.conv_2, name="weight")
            torch.save(G.state_dict(), f"checkpoint_cifar/DPCGAN_CIFAR_eps{eps:.1f}.bin")
            ...
            iterations = 0
            gan_epoch = ((args.iter * args.mini) // len(train_set))+1
                ...
                for i, (imgs, labels) in bar:
                    ...
                    if((iterations+1) %100 ==0):
                        ...
                        if((iterations+1) in log2):
                            ...
                            torch.save(G.state_dict(), f"./checkpoint_cifar/"+dire+f"/diff_iter/iteration{iterations+1}.ckpt")
            ...
            torch.save(G.state_dict(), f"checkpoint_cifar/DPCGAN_CIFAR.bin")
