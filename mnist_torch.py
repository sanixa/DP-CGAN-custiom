...
dir = "eps_1000"
fl1 = open("./checkpoint/"+dir+"/diff_iter/acc_different_iter.txt","w")
fl2 = open("./checkpoint/"+dir+"/diff_acc/iter_acc.txt","w")
fl3 = open("./checkpoint/"+dir+"/record.txt","w")
	...
	if (args.noise > 0):
		...
		for iteration, (x_mini, y_mini) in bar:
			...
			if((iteration+1) %200 ==0):
				...
				C = Classifier().cuda()
				C.train()
				opt_C = torch.optim.Adam(C.parameters(), lr=1e-3,weight_decay=1e-4)
				...
				if(iteration in log2):
					fl2.write(f"iteration:{iteration:5d}\tacc:{test_acc/test_set.__len__()}\n")
					torch.save(G.state_dict(), f"checkpoint/"+dir+"/DPCGAN_MNIST_eps{eps:.1f}_iteration{iteration}.bin")

				for i in range(len(log1)):
					if(test_acc/test_set.__len__() >log1[i] and log1_[i] ==False):
						fl1.write(f"iteration:{iteration:5d}\tacc:{test_acc/test_set.__len__()}\n")
						log1_[i] = True
						aaa = test_acc/test_set.__len__()
						torch.save(G.state_dict(), f"checkpoint/"+dir+"/DPCGAN_MNIST_eps{eps:.1f}_acc{aaa:.1f}.bin")
				del C
		...
		for epoch in range(gan_epoch):
			...
			if((ㄛ+1) %200 ==0):
				...
				print(f'iteration: {i+1} result of test accuracy = {test_acc/test_set.__len__():.3f}')
				if(i in log2):
					fl2.write(f"iteration:{i:5d}\tacc:{test_acc/test_set.__len__()}\n")
					torch.save(G.state_dict(), f"checkpoint/"+dir+"/DPCGAN_MNIST_eps{eps:.1f}_iteration{i}.bin")

				for i in range(len(log1)):
					if(test_acc/test_set.__len__() >log1[i] and log1_[i] ==False):
						fl1.write(f"iteration:{i:5d}\tacc:{test_acc/test_set.__len__()}\n")
						log1_[i] = True
						aaa = test_acc/test_set.__len__()
						torch.save(G.state_dict(), f"checkpoint/"+dir+"/DPCGAN_MNIST_eps{eps:.1f}_acc{aaa:.1f}.bin")
			G.eval()
